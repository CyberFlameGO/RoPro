...
function addUpcomingCard(name, leakid, icon, discovered) {
    console.log(name, leakid, icon, discovered)
...
async function upcomingMain() {
    ...
    if (categoryPanel != null) {
        panelGroup = document.getElementsByClassName('panel-group')
        for (i = 0; i < panelGroup.length; i++) {
            if (panelGroup[i].style.display != "none") {
                categoryPanel = panelGroup[i]
            }
        }
        li = document.createElement('li')
        li.setAttribute("class", "font-header-2 text-subheader panel panel-default ng-scope")
        li.innerHTML += upcomingHTML
        console.log(categoryPanel.getElementsByTagName('li')[0])
